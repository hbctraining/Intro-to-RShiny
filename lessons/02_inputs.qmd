...
:::{.callout-note}
You can alternatively use `selectizeInput()` instead of `selectInput()` to have more control over the dropdown. While this is outside of the scope of this lesson, the documentation for this can be found [here](https://selectize.dev/docs/usage).
:::
...
:::{.callout-tip}
# [**Exercise 1**](02_inputs-Answer_key.qmd#exercise-1)
...
```{r}
#| label: conditional_panel_explanation
#| eval: false
# DO NOT RUN
conditionalPanel(
...
```{r}
#| label: conditional_panel_example
#| eval: false
# User interface
ui <- fluidPage(
