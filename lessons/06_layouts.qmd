...
```{r}
#| label: equal_sized_columnd_example
#| eval: false
library(shiny)
library(ggplot2)
...
Within the navigation bar, we can have a dropdown menu that gives us more options in a list. This would be accomplished using the `navbarMenu()` function, which also utilizes `tabPanel()` function that `navbarPage()` uses. The syntax for using a dropdown menu anvigation bar menu is:

```{r}
#| label: navbar_menu_syntax
#| eval: false
# DO NOT RUN
navbarPage("<title_of_navigation_bar>",
...
:::{.callout-tip}
# [**Exercise 1**](06_layouts-Answer_key.qmd#exercise-1)
